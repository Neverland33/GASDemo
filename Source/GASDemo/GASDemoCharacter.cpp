...
void AGASDemoCharacter::BeginPlay()
{
	...
	if (SelfAbilitySystemComponent)
	{
		...
		SelfAbilitySystemComponent->GetGameplayAttributeValueChangeDelegate(UBaseAttributeSet::GetManaAttribute()).AddUObject(this, &AGASDemoCharacter::OnManaAttributeValueChange);
		SelfAbilitySystemComponent->GetGameplayAttributeValueChangeDelegate(UBaseAttributeSet::GetStrengthAttribute()).AddUObject(this, &AGASDemoCharacter::OnStrengthAttributeValueChange);
	}
}
...
/* 法力值改变时广播 */
void AGASDemoCharacter::OnManaAttributeValueChange(const FOnAttributeChangeData& Data)
{
	ManaChangeEvent.Broadcast(Data.NewValue);
}

/* 能量值改变时广播 */
void AGASDemoCharacter::OnStrengthAttributeValueChange(const FOnAttributeChangeData& Data)
{
	StrengthChangeEvent.Broadcast(Data.NewValue);
}

/* 学习技能时获取技能信息 */
FGameplayAbilityInfo AGASDemoCharacter::GetGameplayAbilityInfo(TSubclassOf<UBaseGameplayAbility> AbilityClass, int level)
{
	UAbilitySystemComponent* SelfAbilitySystemComponent = this->FindComponentByClass<UAbilitySystemComponent>();
	UBaseGameplayAbility* AbilityInstance = AbilityClass->GetDefaultObject<UBaseGameplayAbility>();
	if (SelfAbilitySystemComponent && AbilityInstance)
	{
		return AbilityInstance->GetAbilityInfoByLevel(level);
	}
